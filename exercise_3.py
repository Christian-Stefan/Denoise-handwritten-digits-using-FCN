...
class Fit_Predict():
            ...
            # ... denoised images. 
                ...
                if self.weights_path == None:
                    ...
                    detector, denoiser, report = self.activate_Adversial_Mode(path=[
                        rf'assignment_1_[2036967]_[2036967]\Submission\exercise1.3_firsthalf_results\train_detector_submission\train_detector\train_detectormodel_weights_epoch19.pth',
                        rf'assignment_1_[2036967]_[2036967]\Submission\exercise1.3_firsthalf_results\train_denoiser_submission\train_denoiser\train_denoisermodel_weights_epoch19.pth'
                    ])
    ...
    def activate_Super_Training_Denoiser_Mode(self,epochs,alpha=0.1,detector_path=None,denoiser_path=None):
        ...
        if detector_path is None:
            detector_path = r'assignment_1_[2036967]_[2036967]\Submission\exercise1.3_finalpart_results\training_Super_detector_submission\training_Super_detector\training_Super_detectormodel_weights_epoch29.pth'
        if denoiser_path is None:
            denoiser_path = r'assignment_1_[2036967]_[2036967]\Submission\exercise1.3_finalpart_results\train_denoiser_3_submission\train_denoiser\train_denoisermodel_weights_epoch19.pth'
    ...
    def activate_Super_Training_Detector_Mode(self, epochs):
        ...
        denoiser_path = rf'assignment_1_[2036967]_[2036967]\Submission\exercise1.3_firsthalf_results\train_denoiser_3_submission\train_denoiser\train_denoisermodel_weights_epoch19.pth'
        denoiser.load_state_dict(torch.load(denoiser_path))
        denoiser.eval()
    ...
    def activate_Adversial_Mode(self, path:[str, str], baseline_change=None, save_dir=None, prefix="report", show=True):
        ...
        likelihood_change = self.report(adversial_report,off=False,baseline_change=baseline_change,
                                        save_dir=save_dir,prefix=prefix,show=show)
    ...
    def report(self, history:list|str|dict, off=True, baseline_change=None, save_dir=None, prefix="report", show=True):
        """
        Motivation:
        ...
        if isinstance(history, dict):
            ...
            PLT.boxplot(
                [clean_scores, noisy_scores, denoised_scores],
                labels=["Clean images", "Noisy images", "Denoised images"]
            )

            PLT.ylabel("Likelihood of being noisy")
            PLT.title("Noise detector likelihoods across test image sets")
            PLT.grid(axis="y", alpha=0.3)
            PLT.tight_layout()

            if save_dir is not None:
                PLT.savefig(
                    os.path.join(save_dir, f"{prefix}_three_likelihood_boxplots.png"),
                    dpi=300,
                    bbox_inches="tight"
                )
            ...
            if save_dir is not None:
                PLT.savefig(
                    os.path.join(save_dir, f"{prefix}_likelihood_change_boxplot.png"),
                    dpi=300,
                    bbox_inches="tight"
                )
            ...
            if save_dir is not None:
                PLT.savefig(
                    os.path.join(save_dir, f"{prefix}_fooledness_3x3.png"),
                    dpi=300,
                    bbox_inches="tight"
                )
...
## 2. Keep in mind that some blocks are inextricably linked so running one block of code might mean uncommenting other;
## 3. ...
...
#                     ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
# Uncomment this part vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
...
#     data_loc=r"/DataSets"
# )
# _, _, original_report = fit.activate_Adversial_Mode(path=[rf'{detector}',rf'{denoiser}'], 
#           save_dir=path_1_3_1)
...
>>>>>>> 51a94c7748bef4ebb0e3c948da92c972bc219b45
=======
### ---- START M.L. Specific Imports --- ###
# from xml.parsers.expat import model
from assignment1_data.mnist_dataloader import create_dataloaders
